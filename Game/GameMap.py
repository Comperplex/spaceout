...
class GameMap:
	...
	def addObject(self, gameObject): #Tries to add a new game Object to the dictionary of game objects. Returns true if successful and false otherwise
		if gameObject.loc[0] in range(self.size[0]) and gameObject.loc[1] in range(self.size[1]) and self.getNumEntities() < self.maxEntities:
	...
	def getNumEntities(self):
		...
		for key in self.gameObjects:
			numEntities += len(self.gameObjects[key])
