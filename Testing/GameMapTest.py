...
class TestGameMap(unittest.TestCase):
	...
	def testID(self):
		...
		for i in range(10):
			...
			self.assertEqual(gameMap.gameObjects[i].ID, i) #Testing successful ID assignment

		gameMap.addObject(GameObject([0,0], 'beacon', 'owen'))
		self.assertEqual(gameMap.playerSortedObjectDict['owen'][0].ID, 0) #Testing successful '0' ID assignment

		self.assertTrue(gameMap.removeObject('owen', 'drone', 2)) #Testing successful removal of an object
		self.assertIsNone(gameMap.getSpecificGameObject('owen', 'drone', 2)) #Testing that the object was indeed removed
		self.assertIsNotNone(gameMap.getSpecificGameObject('owen', 'drone', 3)) #Testing that another object that was added indeed exists

		gameMap.addObject(GameObject([1, 1], 'drone', 'owen'))
		self.assertIsNotNone(gameMap.getSpecificGameObject('owen', 'drone', 2)) #Testing that, after adding a new relevant game object, the old ID=2 has been re-used
